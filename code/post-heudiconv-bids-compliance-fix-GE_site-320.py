...
# Set PE direction to 'i' for Right->Left based on DICOM info and visual inspection of raw data 
# ----------------------------
def fix_dwi_json(session_dir: Path, dry_run=False):
    ...
    for json_file in sorted(dwi_dir.glob("*.json")):
        ...
        if data.get("PhaseEncodingDirection") != "i-":
            data["PhaseEncodingDirection"] = "i-"; changed = True
