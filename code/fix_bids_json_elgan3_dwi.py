...
# Modified for ELGAN3 by Meaghan Perdue 10 July 2025
    ...
    if args.diff:
        ...
        for scan in scans:
            ...
            if scan['subdir'] == 'fmap':
                ...
                assert 'B0FieldIdentifier' not in scan.data
                ...
                if scan['dir'] == 'PA':
                    scan.data['PhaseEncodingDirection'] = 'j'
                # print('        Setting IntendedFor')
                # iscans = []
                # assert scan['acq'] in ['ABCD1', 'ABCD2']
                # if scan['acq'] == 'ABCD1': # this should point to both dwi/acq-ABCD1 and dwi/acq-ABCD2
                #     for iscan in scans.iter_subdir('dwi'):
                #         iscans.append(iscan.session_path.as_posix())
                # if scan['acq'] == 'ABCD2': # this should point to both dwi/acq-ABCD1 and dwi/acq-ABCD2
                #     for iscan in scans.iter_subdir('dwi'):
                #         iscans.append(iscan.session_path.as_posix())
                # scan.data['IntendedFor'] = iscans
            if scan['subdir'] == 'dwi':
                print('        Setting PhaseEncodingDirection')
                #assert 'PhaseEncodingDirection' not in scan.data
                assert scan.data['PhaseEncodingAxis'] == 'j'
